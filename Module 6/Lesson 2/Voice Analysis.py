...
def plot_both(stats1, stats2, rate):
    ...
    ax1.set_title(
        f"Recording 1 (Normal) - "
        f"Duration: {stats1['duration']:.2f}s, "
        f"Avg: {stats1['avg_volume']:.0f}"
    )
    ...
    ax2.set_title(
        f"Recording 2 (Modified) - "
        f"Duration: {stats2['duration']:.2f}s, "
        f"Avg: {stats2['avg_volume']:.0f}"
    )
